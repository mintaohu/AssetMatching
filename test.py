...
# Find singe match
def find_mpts(image0, image1, matching, device, num_mkpts):
    max_pair = None
            ...
            if kpts1.shape[0] < 15:
                if matches[valid].shape[0] >= max(4, int(math.floor(kpts1.shape[0]) * 0.7)):
                    if matches[valid].shape[0] > num_mkpts:
                        ...
                        max_pair = (image0[j], image1[i], kpts0, kpts1, mkpts0, mkpts1)
            else:
                if matches[valid].shape[0] >= max(4, int(math.floor(kpts1.shape[0]) / 3)):
                    if matches[valid].shape[0] > num_mkpts:
                        num_mkpts = matches[valid].shape[0]
    ...
    if num_mkpts == 0:
        ...
        return max_pair
        print("BINGO!")
        make_matching_plot(max_pair[0], max_pair[1], max_pair[2], max_pair[3], max_pair[4], max_pair[5], color, True)


def find_match(asset_path, img_path, device, matching):
    color = 'r'
    num_mkpts = 0
    image0, image1 = read_image_test(asset_path, img_path, False)
    max_pair = find_mpts(image0, image1, matching, device, num_mkpts)
...
if __name__ == "__main__":
    ...
    find_match(args.asset, args.img, device, matching)
